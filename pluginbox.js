...
(function(window, document) {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .delete-button {
                background-color: #d81b60; /* 紫红色删除按钮 */
            }
            ...
            .toggle-button {
                background-color: #9c27b0; /* 紫色启用按钮 */
            }
            ...
            .toggle-button.disabled {
                background-color: #ce93d8; /* 淡紫色禁用按钮 */
                color: #f3e5f5; /* 禁用状态文字颜色 */
            }

            #no-plugins-message {
                text-align: center;
                color: #9c27b0; /* 提示信息为中紫色 */
                padding: 20px;
                font-style: italic;
                font-size: 0.9em;
                margin: 0;
            ...
            <div class="viewer-content">
                ...
                <div id="add-plugin-form">
                    <div class="form-group">
                        <label for="plugin-file">选择JS插件文件</label>
    ...
    class PluginSystem {
        constructor() {
            ...
            // 设置默认位置为右下角
            this.DEFAULT_BUTTON_LEFT = 'auto';
            this.DEFAULT_BUTTON_RIGHT = 20;
            this.DEFAULT_BUTTON_TOP = 'auto';
            this.DEFAULT_BUTTON_BOTTOM = 20;
        ...
        initButtonDrag() {
            ...
            document.addEventListener('mousemove', (e) => {
                if (this.isDraggingButton) {
                    ...
                    // 使用left和top定位，清除right和bottom
                    this.puzzleButton.style.left = `${constrainedLeft}px`;
                    this.puzzleButton.style.top = `${constrainedTop}px`;
                    this.puzzleButton.style.right = 'auto';
                    this.puzzleButton.style.bottom = 'auto';
        ...
        loadButtonPosition() {
            ...
            if (buttonPosition) {
                try {
                    const { left, top, right, bottom } = JSON.parse(buttonPosition);
                    // 如果有right和bottom值，优先使用它们
                    if (right !== undefined && bottom !== undefined) {
                        this.puzzleButton.style.left = left || 'auto';
                        this.puzzleButton.style.right = right;
                        this.puzzleButton.style.top = top || 'auto';
                        this.puzzleButton.style.bottom = bottom;
                    } else {
                        this.puzzleButton.style.left = left;
                        this.puzzleButton.style.top = top;
                        this.puzzleButton.style.right = 'auto';
                        this.puzzleButton.style.bottom = 'auto';
                    }
                } catch (e) {
                    ...
                    // 加载失败时使用默认位置
                    this.puzzleButton.style.left = this.DEFAULT_BUTTON_LEFT;
                    this.puzzleButton.style.right = this.DEFAULT_BUTTON_RIGHT;
                    this.puzzleButton.style.top = this.DEFAULT_BUTTON_TOP;
                    this.puzzleButton.style.bottom = this.DEFAULT_BUTTON_BOTTOM;
                }
            } else {
                // 如果没有保存的位置，使用默认右下角位置
                this.puzzleButton.style.left = this.DEFAULT_BUTTON_LEFT;
        ...
        resetButtonPosition() {
            ...
            localStorage.setItem('puzzle_button_position', JSON.stringify({
                ...
                right: this.DEFAULT_BUTTON_RIGHT,
                top: this.DEFAULT_BUTTON_TOP,
                bottom: this.DEFAULT_BUTTON_BOTTOM
            }));
        ...
        updatePluginList() {
            ...
            this.plugins.forEach((plugin, index) => {
                ...
                statusDiv.style.color = plugin.disabled ? '#b39ddb' : '#7b1fa2'; // 状态文字使用紫色系
        ...
        installPlugin(code) {
            ...
            if (result.error) {
                this.showError('plugin-file-error', result.error);
                return false;
            }
            ...
            if (this.isPluginInstalled(packname)) {
                this.showError('plugin-file-error', `已安装同名插件: ${packname}`);
                return false;
            }
            ...
            if (!depCheck.valid) {
                this.showError('plugin-file-error', `缺少依赖插件: ${depCheck.missing.join(', ')}`);
                return false;
            }
            ...
            if (!compatCheck.valid) {
                this.showError('plugin-file-error', compatCheck.reason);
                return false;
            }
            ...
            try {
                ...
                this.showError('plugin-file-error', `插件运行出错: ${e.message}`);
                return false;
            }
        ...
        notify(message, type = 'info') {
            ...
            notification.style.color = 'white'; // 通知文字为白色，与背景对比
            ...
            switch(type) {
                case 'success':
                    notification.style.backgroundColor = '#7b1fa2'; // 深紫色
                    break;
                case 'error':
                    notification.style.backgroundColor = '#c2185b'; // 紫红色
                    break;
                case 'warning':
                    notification.style.backgroundColor = '#9c27b0'; // 中紫色
                    break;
                default:
                    notification.style.backgroundColor = '#9c27b0'; // 默认中紫色
            }
